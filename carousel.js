...
class CarouselController {
    ...
    cacheElements() {
        ...
        this.layer3a = this.carousel.querySelector('.carousel-layer-3a');
        this.layer3b = this.carousel.querySelector('.carousel-layer-3b');
        ...
        this.layer3aImage = this.layer3a.querySelector('.zoomable-image');
        this.layer3aMenu = this.layer3a.querySelector('.carousel-menu');
        this.layer3aButtons = this.layer3aMenu.querySelectorAll('button');
        
        this.layer3bImage = this.layer3b.querySelector('.zoomable-image');
        this.layer3bMenu = this.layer3b.querySelector('.carousel-menu');
        this.layer3bButtons = this.layer3bMenu.querySelectorAll('button');
    ...
    init() {
        ...
        this.setupLayer3aEvents();
        this.setupLayer3bEvents();
    ...
    setFocusForLayer(layerId) {
        switch(layerId) {
            ...
            case '3':
                ...
                break;
            case '3a':
                this.layer3aButtons[0].focus();
                break;
            case '3b':
                this.layer3bButtons[0].focus();
                break;
        }
    }
    ...
    updateCarouselDisplay() {
        ...
        // Update Layer 3, 3a, 3b images
        this.layer3Image.src = currentImage.src;
        this.layer3Image.alt = currentImage.alt;
        this.layer3aImage.src = currentImage.src;
        this.layer3aImage.alt = currentImage.alt;
        this.layer3bImage.src = currentImage.src;
        this.layer3bImage.alt = currentImage.alt;
    }
    ...
    enterLayer3() {
        ...
        const image = this.images[this.state.currentSlide];
        this.announce(`Zoom mode. ${image.alt}. Full screen. Exit, Pan, or Zoom.`);
    }
    ...
    setupLayer3Events() {
        ...
        this.layer3Buttons.forEach(button => {
            button.addEventListener('click', () => {
                if (this.state.currentLayer !== '3') return;
                this.handleLayer3Action(button.dataset.action);
            });
    ...
    enterLayer3a() {
        ...
        this.announce('Pan controls. Exit, Up, Down, Left, Right, Reset.');
    }
    
    setupLayer3aEvents() {
        // Focus trap
        this.layer3a.addEventListener('keydown', (e) => {
            if (this.state.currentLayer !== '3a') return;
            this.handleFocusTrap(e, this.layer3aButtons);
        });
        
        // Button actions with hold-to-repeat
        this.layer3aButtons.forEach(button => {
            const action = button.dataset.action;
            
            // Click handler
            button.addEventListener('click', () => {
                if (this.state.currentLayer !== '3a') return;
                this.handleLayer3aAction(action);
            });
            
            // Keydown for Enter - with hold-to-repeat for pan directions
            button.addEventListener('keydown', (e) => {
                if (this.state.currentLayer !== '3a') return;
                if (e.key === 'Enter') {
                    e.preventDefault();
                    
                    // For pan directions, start hold-to-repeat
                    if (['pan-up', 'pan-down', 'pan-left', 'pan-right'].includes(action)) {
                        if (!this.state.holdTimer) {
                            this.handleLayer3aAction(action);
                            this.startHoldToRepeat(() => this.handleLayer3aAction(action));
                        }
                    } else {
                        this.handleLayer3aAction(action);
                    }
    ...
    resetPan() {
        ...
        this.updateZoomTransform();
        this.updatePanButtonStates();
        this.announce('Reset. Image centered.');
    }
    
    exitToLayer3() {
        this.setLayer('3');
        this.announce('Exited pan controls.');
    }
    ...
    enterLayer3b() {
        ...
        this.announce('Zoom controls. Exit, Zoom In, Zoom Out, Reset.');
    }
    
    setupLayer3bEvents() {
        // Focus trap
        this.layer3b.addEventListener('keydown', (e) => {
            if (this.state.currentLayer !== '3b') return;
            this.handleFocusTrap(e, this.layer3bButtons);
        });
        
        // Button actions
        this.layer3bButtons.forEach(button => {
            button.addEventListener('click', () => {
                if (this.state.currentLayer !== '3b') return;
                this.handleLayer3bAction(button.dataset.action);
            });
    ...
    handleLayer3bAction(action) {
        switch(action) {
            case 'exit':
                this.exitToLayer3FromZoom();
                break;
            case 'zoom-in':
                this.zoomIn();
                break;
    ...
    updateZoomTransform() {
        ...
        // Apply to all zoomable images
        [this.layer3Image, this.layer3aImage, this.layer3bImage].forEach(img => {
            img.style.transform = transform;
        });
    }
    
    exitToLayer3FromZoom() {
        this.setLayer('3');
        this.announce('Exited zoom controls.');
    }
